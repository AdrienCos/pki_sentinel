...
$OPENSSL_BIN req -new -config $DIR/$ROOT_NAME.conf -key $DIR/$ROOT_NAME.key -out $DIR/$ROOT_NAME.csr

# Self-sign the certificate request
echo "Generating Certificate: $DIR/$ROOT_NAME.crt"
$OPENSSL_BIN ca -selfsign -days 9999 -config $DIR/$ROOT_NAME.conf -in $DIR/$ROOT_NAME.csr -out $DIR/$ROOT_NAME.crt -batch

# Generate a PEM from the certificate file output
echo "Generating PEM: $DIR/$ROOT_NAME.pem"
$OPENSSL_BIN x509 -in $DIR/$ROOT_NAME.crt -outform pem -out $DIR/$ROOT_NAME.pem
